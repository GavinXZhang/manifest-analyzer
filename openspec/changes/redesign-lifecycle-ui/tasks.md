...
- [ ] 2.1 Additive migrations in `src/store/db.ts`: `lots.stage`, `lot_stage_events`, `receipts`, `parts_book`, `channels`, `listings`, `punches`, `recurring_expenses`; new columns on `inventory` (`kind`, `family`, `line_item_id`, `received_at`, `qty_sold`) and `sales` (`inventory_id`, `channel_id`, `fees`)
- [ ] 2.2 Back-fill `lots.stage` and one inferred stage event per existing lot; migrate `storage_units → recurring_expenses`, `work_hours`/`timer → punches`; seed `channels` and `parts_book`; run against a copy of the production database and check row counts
- [ ] 2.3 `src/store/stages.ts`: set stage + append event; hook outcome save (won → won, lost → closed); unit tests
- [ ] 2.4 `postDueRecurring(today)` in `src/store/recurring.ts`, idempotent per period with back-fill; unit tests for due day, double run, missed month

## 3. Lots board (replaces Lots + Compare)

- [ ] 3.1 `GET /api/lots/board` returning lots grouped by stage with per-column totals and days-in-stage
...
- [ ] 4.1 `src/store/receipts.ts` + routes `GET/PUT /api/lots/:id/receipts` with the tally validation (parts ≤ received); progress summary in the lot response
- [ ] 4.2 `src/valuation/families.ts`: parts-book matching of manifest lines; `src/calc/salvage.ts`: per-family pools, cannibalisation count, floors, lot floor; unit tests against the vacuum manifest fixture
- [ ] 4.3 Transfer routine "send working units to inventory" (idempotent per manifest line, cost = landed unit cost, kind `unit`, family) and "create parts items" (kind `parts`, cost 0); routes and tests
- [ ] 4.4 `GET /api/lots/:id/salvage-plan`; "Finish check-in" → stage `received`
...
- [ ] 4.6 Parts book store + `GET/PUT /api/parts-book`; seed from the exploration salvage table with the "estimate" label

## 5. Inventory, channels, listings

- [ ] 5.1 `src/store/channels.ts` and `src/store/listings.ts` with routes: `GET/PUT /api/channels`, `POST/PUT/DELETE /api/inventory/:id/listings`
- [ ] 5.2 `src/calc/fees.ts`: net after fees (percent + fixed, local/shipped rule), best net; derived inventory status written on every change; unit tests
- [ ] 5.3 Sales gain `inventory_id`, `channel_id`, `fees`; "mark sold" endpoint decrements `qty_sold`, marks the listing sold, records fees; first listing on a lot's item → stage `selling`; all sold → close suggestion
- [ ] 5.4 `src/calc/aging.ts`: days on shelf, carrying cost per item-day, accrued cost, threshold flags; "cut price" endpoint updating active asks; unit tests
- [ ] 5.5 Listing drafts: templates per draft style; wire the existing Claude drafting endpoint when a key is configured
...
- [ ] 6.1 `src/store/punches.ts`: clock in (refuse if one running), clock out, switch task, manual add/edit/delete; routes under `/api/punches`; unit tests
- [ ] 6.2 Aggregations: hours per day/week, by category, profit per hour for a period, labor cost per lot; tests
...
- [ ] 7.1 `GET /api/reports/:kind` for revenue-goal, net, funnel, cycle, hours, storage with period (30d/90d/ytd/all), by, lot; previous-period comparison for scorecards; fixture-based tests
...
- [ ] 8.1 `GET /api/today` composing punch state, check-in lots, month-to-date scorecards, aging items, upcoming events, recent sales; runs recurring posting first
...
- [ ] 9.2 CSV export endpoints for lots, inventory, listings, sales, expenses, punches
